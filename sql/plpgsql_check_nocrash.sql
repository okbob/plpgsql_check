...
CREATE OR REPLACE FUNCTION repro05_a() RETURNS void AS $$
BEGIN
    RAISE NOTICE '%', format('%2$s', 'a');
END
$$ LANGUAGE plpgsql;

-- buggy: reports nothing
-- correct: error:22023:...:RAISE:too few arguments for format()
SELECT * FROM plpgsql_check_function('repro05_a()');

-- for comparison, the same mistake without an explicit position IS reported
CREATE OR REPLACE FUNCTION repro05_a2() RETURNS void AS $$
BEGIN
    RAISE NOTICE '%', format('%s %s', 'a');
END
$$ LANGUAGE plpgsql;

SELECT * FROM plpgsql_check_function('repro05_a2()');

----------------------------------------------------------------------
-- part B: missed SQL-injection warning
----------------------------------------------------------------------

CREATE OR REPLACE FUNCTION repro05_b(p text) RETURNS void AS $$
BEGIN
    EXECUTE format('SELECT %1$s', p);
END
$$ LANGUAGE plpgsql;

-- buggy: reports nothing
-- correct: security:00000:...:EXECUTE:text type variable is not sanitized
SELECT * FROM plpgsql_check_function('repro05_b(text)', security_warnings => true);

-- for comparison, the same code without an explicit position IS reported
CREATE OR REPLACE FUNCTION repro05_b2(p text) RETURNS void AS $$
BEGIN
